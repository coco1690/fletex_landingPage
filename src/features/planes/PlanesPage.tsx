...
import { usePlanesStore, type PlanSuscripcion } from './planesStore'
...
import { POR_PAGINA } from './planesStore'
...
export function PlanesPage() {
  ...
  return (
    <>
      ...
      >
        {filtrados.length === 0 ? (
          <TablaVacia cargando={cargando} hayDatos={planes.length > 0}
            hayBusqueda={!!busqueda || filtroEstado !== 'todos'}
            icono={<CreditCard className="w-6 h-6" />}
            labelVacio="Sin planes de suscripción" labelCrear="Nuevo plan" onCrear={handleCrear} />
        ) : (
          <>
            {/* Móvil */}
            <div className="md:hidden overflow-y-auto max-h-[calc(100dvh-370px)] divide-y divide-border">
              {filtrados.map(p => (
                <button key={p.id} onClick={() => handleSeleccionar(p)}
                  className={`w-full text-left p-4 transition-colors ${planDetalle?.id === p.id ? 'bg-primary/5' : 'hover:bg-secondary/50'}`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-semibold text-foreground">{p.nombre}</span>
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                      p.activo ? 'bg-success/10 text-success border-success/20' : 'bg-destructive/10 text-destructive border-destructive/20'
                    }`}>{p.activo ? 'Activo' : 'Inactivo'}</span>
                  </div>
                  <p className="text-lg font-black text-primary">${p.precio.toLocaleString('es-CO')}</p>
                  <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{p.duracion_dias} días</span>
                    <span>{p.moneda}</span>
                  </div>
                </button>
              ))}
                ...
                <tbody className="divide-y divide-border">
                  {filtrados.map(p => (
                    <tr key={p.id} onClick={() => handleSeleccionar(p)}
                      className={`cursor-pointer transition-colors ${planDetalle?.id === p.id ? 'bg-primary/5' : 'hover:bg-secondary/50'}`}>
                      <td className="py-3 px-4">
                        <div>
                          <p className="font-medium text-foreground">{p.nombre}</p>
                          {p.descripcion && <p className="text-[10px] text-muted-foreground truncate max-w-[200px]">{p.descripcion}</p>}
                        </div>
                      </td>
                      <td className="py-3 px-4 text-right font-bold text-lg text-primary">${p.precio.toLocaleString('es-CO')}</td>
                      <td className="py-3 px-4 text-center text-muted-foreground">{p.duracion_dias} días</td>
                      <td className="py-3 px-4 text-center">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                          p.incluye_planilla ? 'bg-info/10 text-info border-info/20' : 'bg-muted text-muted-foreground border-border'
                        }`}>{p.incluye_planilla ? 'Sí' : 'No'}</span>
                      </td>
                      <td className="py-3 px-4">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                          p.activo ? 'bg-success/10 text-success border-success/20' : 'bg-destructive/10 text-destructive border-destructive/20'
                      ...
                      <td className="py-3 px-4 text-right" onClick={e => e.stopPropagation()}>
                        <MenuAcciones items={[
                          { label: 'Editar', icon: <Pencil className="w-3.5 h-3.5" />, fn: () => handleEditar(p) },
                          { label: p.activo ? 'Desactivar' : 'Activar', icon: <ToggleLeft className="w-3.5 h-3.5" />, fn: () => toggleEstado(p.id, p.activo) },
                        ]} />
                      </td>
                    </tr>
                  ))}
      ...
      {modalAbierto && (
        <ModalPlan plan={planEditando} onGuardar={crearPlan} onActualizar={actualizarPlan}
          onCerrar={() => { setModalAbierto(false); setPlanEditando(null) }}
          cargando={cargando} usuarioId={usuario?.id ?? ''} />
      )}
    </>
  )
}
